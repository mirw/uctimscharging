/* $Id: lock_ops.h 153 2007-02-26 14:42:23Z vingarzan $ */
/*
 *
...
{
	//LOG(L_NOTICE,"in the get lock, just in\n");
	LOG(L_INFO,"IMS_AS - in the get lock, just in %d\n", lock);
	struct sembuf sop;
	//LOG(L_NOTICE,"in the get lock, just dec\n");
	sop.sem_num=0;
	//LOG(L_NOTICE,"in the get lock, sem 0\n");
	sop.sem_op=-1; // down 
	//LOG(L_NOTICE,"in the get lock, sem down\n");
	sop.sem_flg=0; 
	//LOG(L_NOTICE,"in the get lock, flag 0\n");
	semop(*lock, &sop, 1);
	//LOG(L_NOTICE,"in the get lock, leaving\n");
}
