...
int peer_connect(peer *p)
{
	...
	if (error!=0){
		LOG(L_ERR,"WARNING:peer_connect(): Error opening connection to %.*s:%d >%s\n",
	...
	for(ainfo = res;ainfo;ainfo = ainfo->ai_next)
	{
		if (getnameinfo(ainfo->ai_addr,ainfo->ai_addrlen,
			host,256,serv,256,NI_NUMERICHOST|NI_NUMERICSERV)==0){
				LOG(L_INFO,"INFO:peer_connect(): Trying to connect to %s port %s\n",
					host,serv);
		}				
		...
		if (connect(sock,ainfo->ai_addr,ainfo->ai_addrlen)!=0) {
			LOG(L_INFO,"WARNING:peer_connect(): Error opening connection to to %s port %s >%s\n",
				host,serv,strerror(errno));
			close(sock);		
			continue;
		}
		...
		#endif
		if (pid<0){
			LOG(L_INFO,"ERROR:peer_connect(): fork() failed > %s\n",strerror(errno));
			goto error;
		}
		if (pid==0){
			/* child */
			receiver_process(sock);
			LOG(L_ERR,"ERROR:peer_connect(): receiver_process finished without exit!\n");
			exit(-1);
		}else{
			/* parent */
