...
int trans_init()
{
	LOG(L_INFO,"trans_init() - Initializing transaction list\n");
...
inline cdp_trans_t* add_trans(AAAMessage *msg,AAATransactionCallback_f *cb, void *ptr,int timeout,int auto_drop)
{
	...
	//LOG(L_INFO,"INFO: add_trans() - pointer = %d\n",ptr);
	x->cb = cb;
	*(x->ptr) = ptr;
	...
	//LOG(L_INFO,"Trans add - We got here\n");
	lock_get(trans_list->lock);
	//LOG(L_INFO,"Trans add - We are here\n");
