 ...
 */
/* I guess we can wait for sip events in here as well as aaa messages*/
void accept_loop()
{
	fd_set listen_set;
	...
	while(1){
		...
		if (nready == 0){
			...
			//printf("why wont mike leave me alone\n");
			//printf("this means that none of the sockets had anything right?\n");
			continue;
		}
		...
		i=0;
		/* in here we jump to accept_connection above, there was indeed
		 * a connection in one of the sockets. */
		while(listening_socks[i]){
