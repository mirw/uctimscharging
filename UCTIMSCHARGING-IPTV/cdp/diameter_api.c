...
{
	//LOG(L_NOTICE,"sendrecv_cb ever called\n");
	//LOG(L_INFO,"sendrecv_cb ever called\n");
...
AAAMessage* AAASendRecvMessageToPeer(AAAMessage *message, str *peer_id)
{
	...
	if (is_req(message)){
		lock = lock_alloc();
		//LOG(L_INFO,"The lock is %d:\n",lock);
		lock = lock_init(lock);
		//LOG(L_INFO,"The lock is %d:\n",lock);
		lock_get(lock);
		//LOG(L_INFO,"The lock is %d:\n",lock);
		//LOG(L_NOTICE,"until before add_trans\n");
		t = add_trans(message,sendrecv_cb,(void*)lock,DP_TRANS_TIMEOUT,0);
		//LOG(L_NOTICE,"until after add_trans\n");
		...
		ans = t->ans;
		free_trans(t);
		//LOG(L_NOTICE,"we return AAASendRecvmMessageToPeer \n");
		return ans;
	}
	else
	{
