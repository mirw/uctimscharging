...
/**
 * Creates and adds an Accounting Record Type AVP.
 * @param msg - the Diameter message to add to.
 * @param data - the value for the AVP payload
 * @returns 1 on success or 0 on error
 */
 ...
 * @returns 1 on success or 0 on error
 */
inline int Rf_get_Credit_persec(AAAMessage *msg, int *data)
{
	AAA_AVP *avp;
	avp = Rf_get_avp(msg,AVP_Credit_persec,0);
	if (!avp)
	{
	LOG(L_ERR,"ERR:Rf_get_Credit_onceoff: AVP_Credit_persec not found\n");
	return 0;
	}
	*data = get_4bytes(avp->data.s); 
	return 1;
