...
void change_billing_timer(int value, int state)
{
	timer_value = value;
	timer_running = state;
	LOG(L_CRIT,"change billing timer called %d and %d ...\n", timer_value, timer_running);
}
...
/*	lock_get(msg_id_lock);	*/
	...
	LOG(L_DBG,"DBG:peer_manager_destroy(): ...Peer Manager destroyed\n");
}
...
{
	peer *p,*n;
	LOG(L_DBG,":peer_timer(): taking care of peers...\n");
	lock_get(peer_list_lock);
	p = peer_list->head;
	while(p){
		lock_get(p->lock);
	...
	switch (timer_running)
	{
		case 0:
			...
			{
				has_ran = 0;
				LOG(L_NOTICE,"Summary usage: user used %d credits\n", usage_units);
			}
			
			//LOG(L_NOTICE,"case 0 %d credits\n", usage_units);
			
		break;
		
		case 1:
			if (has_ran==0) has_ran = 1;
			if (has_ran==1){
				usage_units++;
				timeout--;
				
			}
			//if ( timeout < 0) timer_running = 0;
			//LOG(L_NOTICE,"case 1 %d credits\n", usage_units);
