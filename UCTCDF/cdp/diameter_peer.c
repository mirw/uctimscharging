/**
 * $Id: diameter_peer.c 169 2007-03-06 13:30:31Z vingarzan $
 *  
...
{	pid_list_t *i,*j; //diameter_peer.h

	config = parse_dp_config(cfg_filename); //config declared above <configparser.c>
	if (!config) {
		LOG(L_ERR,"ERROR:diameter_peer_init(): Error loading configuration file. Aborting...\n");
		goto error;
	}
	log_dp_config(L_INFO,config);
//printf("We just loaded the config file\n");
	
	dp_first_pid = shm_malloc(sizeof(pid_t)); //dp_first_pid declared above
	if (!dp_first_pid){
...
/******
Added by Vitalis
To initialize the transactions
************/
	trans_init();
	// add callback for messages - used to implement the API 
	cb_add(api_callback,0);
...
int diameter_peer_start(int blocking)
{
	...
	#endif
	if (pid==-1){
		LOG(L_CRIT,"ERROR:diameter_peer_start(): Error on fork() for acceptor!\n");
		return 0;
	}
	if (pid==0) {
		acceptor_process(config); //acceptor.c
		LOG(L_CRIT,"ERROR:diameter_peer_start(): acceptor_process finished without exit!\n");
		exit(-1);		
	}else{
		dp_add_pid(pid); //function exists above
	}
	...
	for(k=0;k<config->workers;k++){
		...
		#endif
		if (pid==-1){
			LOG(L_CRIT,"ERROR:diameter_peer_start(): Error on fork() for worker!\n");
			return 0;
		}
		if (pid==0) {
			srandom(time(0)*k);
			...
			#endif	
			worker_process(k);
			LOG(L_CRIT,"ERROR:diameter_peer_start(): worker_process finished without exit!\n");
			exit(-1);		
		}else{
			dp_add_pid(pid);
		}
	...
	if (blocking) {
		...
		#endif
		if (pid==-1){
			LOG(L_CRIT,"ERROR:diameter_peer_start(): Error on fork() for timer!\n");
			return 0;
		}
		if (pid==0) {
			timer_process(0);
			LOG(L_CRIT,"ERROR:diameter_peer_start(): timer_process finished without exit!\n");
			exit(-1);		
		}else{			
			dp_add_pid(pid);
		}
...
void diameter_peer_destroy()
{
	...
	LOG(L_INFO,"INFO:diameter_peer_destroy(): Terminating all childs...\n");
	while(pid_list->tail){
		pid = dp_last_pid();
		if (pid<=0||pid==getpid()){
			dp_del_pid(pid);
		...
		LOG(L_INFO,"INFO:diameter_peer_destroy(): Waiting for child [%d] to terminate...\n",pid);
	...
	LOG(L_INFO,"INFO:diameter_peer_destroy(): All processes terminated. Cleaning up.\n");
...
/*	lock_get(pid_list_lock);*/
	...
	LOG(L_CRIT,"INFO:diameter_peer_destroy(): Bye Bye from C Diameter Peer test\n");
