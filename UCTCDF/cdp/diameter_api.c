...
{
	peer *p;
	p = get_routing_peer(message); //routing.c - struct peer
	if (!p) {
	...
	if (!peer_send_msg(p,message))//receiver.c - int
		goto error;
...
{
	peer *p;
	p = get_peer_by_fqdn(peer_id); //peermanager.c - struct peer 
	if (!p) {
	...
	if (callback_f){
		if (is_req(message))
			add_trans(message,callback_f,callback_param,DP_TRANS_TIMEOUT,1); //transaction.c - cdp_trans_t
		else
...
AAAMessage* AAASendRecvMessage(AAAMessage *message)
{
	...
	gen_lock_t *lock;
	cdp_trans_t *t;
	AAAMessage *ans; //diameter.h
	
	p = get_routing_peer(message); //routing .c
	if (!p) {
	...
	if (is_req(message)){
		...
		lock = lock_init(lock);
		lock_get(lock);
		t = add_trans(message,sendrecv_cb,(void*)lock,DP_TRANS_TIMEOUT,0); //transaction.c

		if (!peer_send_msg(p,message)) { //receiver.c
			lock_destroy(lock);
...
AAAMessage* AAASendRecvMessageToPeer(AAAMessage *message, str *peer_id)
{
	...
	p = get_peer_by_fqdn(peer_id); //peermanager.c
	if (!p) {
