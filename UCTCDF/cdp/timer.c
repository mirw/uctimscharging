...
/*
The following Changes are made to refine the timer resulution from a minimum of 1 second
to a minimum of 1 milisecond - set in timer.c
A theoretical minimum resolution of 1 microsecond may be achieved by these changes
Changes - changed from the use of time() to gettimeofday()
Changes made by Vitalis Ozianyi From the University of Cape Town

The affected files for the Fokus cdp are:
globals.h
timer.c : int add_timer(); void timer_loop()
transaction.c : trans_init();
peermanager.c : peer_manager_init();
*/
#define u_TIMER_RESOLUTION 1000 /**Number of microseconds to wait 1 milisec = 1000 micro-sec, set to lower value, a minimum of 100 microsec recomended **/
...
void timer_loop()
{
	...
	//struct temp_start_time, temp_end_time;
	//int uend_time;
	//int u_temp_start_time, u_temp_end_time;
	timer_cb_t *i;
	callback_f cb=0;
	void *ptr=0;
	...
	while(1){
		...
		//uend_time = (end_time.tv_sec)*1000000 + end_time.tv_usec;
		//LOG(L_DBG,"DBG:timer_loop(): The time is %u\n",(unsigned int)now);
		...
		do {
			...
			//while(i && i->expires>uend_time) i = i->next;
			if (i){
				cb = i->cb;
				ptr = *(i->ptr);
				if (i->one_time){
					...
					//i->expires = uend_time + i->interval;
				}
			}
			lock_release(timers_lock);
	
			if (cb) cb(now,ptr);
			//if (cb) cb(uend_time,ptr);
	
		} while(cb);
		
		//gettimeofday(&temp_start_time, NULL);
		//u_temp_start_time = (temp_start_time.tv_sec)*1000000 + temp_start_time.tv_usec;
		//gettimeofday(&temp_end_time, NULL);
		//u_temp_end_time = (temp_end_time.tv_sec)*1000000 + temp_end_time.tv_usec;
		//while((u_temp_end_time - u_temp_start_time) < u_TIMER_RESOLUTION)
		//{
			//gettimeofday(&temp_end_time, NULL);
			//u_temp_end_time = (temp_end_time.tv_sec)*1000000 + temp_end_time.tv_usec;
			//LOG(L_DBG,"Waiting for next timming cycle\n");
		//}
		//usleep(u_TIMER_RESOLUTION);		
