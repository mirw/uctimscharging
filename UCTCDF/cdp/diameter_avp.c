...
AAA_AVP*  AAACreateAVP(
	...
	char   *data,
	size_t length,
	AVPDataStatus data_status) //diameter_api.h
{
	AAA_AVP *avp;
...
str AAAGroupAVPS(AAA_AVP_LIST avps)
 {
	...
	if (!buf.s) {
		LOG(L_ERR,"ERROR:AAAGroupAVPS: no more free memory!\n");
		return buf;
	}
	...
	if ((char*)p-buf.s!=buf.len) {
		LOG(L_ERR,"BUG:AAAGroupAVPS: mismatch between len and buf!\n");
		shm_free( buf.s );
		buf.s = 0;
		buf.len = 0;
		return buf;
...
AAA_AVP_LIST AAAUngroupAVPS(str buf)
{
	...
	while (ptr < buf.s+buf.len) {
		if (ptr+AVP_HDR_SIZE(0x80)>buf.s+buf.len){
			LOG(L_ERR,"ERROR:AAAUngroupAVPS: source buffer to short!! "
		...
		ptr += AVP_LENGTH_SIZE;
		if (avp_len<1) {
			LOG(L_ERR,"ERROR:AAAUngroupAVPS: invalid AVP len [%d]\n",
				avp_len);
			goto error;
		}
		/* avp vendor-ID */
		...
		if (!avp) {
			LOG(L_ERR,"ERROR:AAAUngroupAVPS: can't create avp for member of list\n");
			goto error;
		}
	...
	AAASearchType searchType)
{
	...
	if (startAvp) {
		...
		if (!avp_t) {
			LOG(L_ERR,"ERROR: AAAFindMatchingAVPList: the \"position\" avp is not "
				"in \"avpList\" list!!\n");
			goto error;
		}
		avp_t=startAvp;
