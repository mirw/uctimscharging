...
AAATransaction *AAACreateTransaction(AAAApplicationId app_id,AAACommandCode cmd_code);  //diameter.h
int AAADropTransaction(AAATransaction *trans);

AAASessionId AAACreateSession(); //diameter.h
...
int AAAAddRequestHandler(AAARequestHandler_f *f,void *param); //diameter.h
int AAAAddResponseHandler(AAAResponseHandler_f *f,void *param); //diameter.h

				/* MESSAGE SENDING */

AAAReturnCode AAASendMessage(	
		AAAMessage *message,
		AAATransactionCallback_f *callback_f, //diameter.h
		void *callback_param);
...
AAAMessage *AAANewMessage( //diameter_msg.c
...
AAA_AVP* AAACreateAVP(
		...
		char *data,
		size_t length,
		AVPDataStatus data_status); //diameter_avp.c
