...
AAAReturnCode AAABuildMsgBuffer( AAAMessage *msg )
{
	...
	*p = (unsigned char)msg->flags;
	...
	if ((char*)p-msg->buf.s!=msg->buf.len) {
		LOG(L_ERR,"BUG: AAABuildMsgBuffer: mismatch between len and buf!\n");
		shm_free( msg->buf.s );
		msg->buf.s = 0;
		msg->buf.len = 0;
		goto error;
...
{
	LOG(L_INFO,"DBG:AAAFreeMessage: Freeing message (%p) %d\n",*msg,(*msg)->commandCode);
	/* param check */
	if (!msg || !(*msg))
		goto done;
...
/**
 *  Sets the proper result_code into the Result-Code AVP; this avp must already
