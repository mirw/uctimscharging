...
typedef struct _routing_entry {
	...
	struct _routing_entry *next;		/** Next route in the routing table 		*/
} routing_entry;
...
dp_config *new_dp_config(); //config.c
routing_realm *new_routing_realm();//config.c
routing_entry *new_routing_entry();//config.c
void free_dp_config(dp_config *x);//config.c
void free_routing_realm(routing_realm *rr);//config.c
void free_routing_entry(routing_entry *re);//config.c
inline void log_dp_config(int level,dp_config *x);//config.c

dp_config* parse_dp_config(char* filename); //in configparse.c
