...
void receive_loop(int sock)
{
    ...
    while(!*shutdownx){
    	...
    	//printf("hopefully we will get here one day\n");
    	
    	dmsg = AAATranslateMessage((unsigned char*)msg,(unsigned int)msg_len,1);
    	
    	/*shm_free(msg);*/

		/* if all went well then jump to this above/below*/
...
/* i get stuck here */
/* well alot of calling of the sm_process() function
 * which is in peerstatemachine() so go there */
void receive_message(AAAMessage *msg,int sock)
{
	AAA_AVP *avp1,*avp2;
